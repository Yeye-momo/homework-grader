              ...
              <div id="canvas-wrap" style={{ position: "relative", maxHeight: "calc(100vh - 100px)", overflow: "auto", background: "#eee", borderRadius: 10, border: "1px solid #E8E8E4" }} onContextMenu={e => e.preventDefault()} onDragStart={e => e.preventDefault()}>
                <div style={{ position: "sticky", top: 0, zIndex: 20, padding: "4px 6px", display: "flex", flexDirection: "column", gap: 3 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                    ...
                    {customStamps.map((s, i) => (<div key={i} style={{ position: "relative", flex: "1 1 0", minWidth: 0, display: "flex" }}>
                      ...
                      {stampEditing && <button onClick={() => setCustomStamps(prev => prev.filter((_, j) => j !== i))} style={{ position: "absolute", top: -6, right: -6, width: 16, height: 16, borderRadius: "50%", border: "none", background: RED, color: "#fff", fontSize: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, padding: 0, zIndex: 2 }}>✕</button>}
                    </div>))}
                    ...
                    {stampEditing && <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", background: "rgba(255,255,255,0.95)", borderRadius: 10, backdropFilter: "blur(8px)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                      <input value={stampAddLabel} onChange={e => setStampAddLabel(e.target.value)} placeholder="新标语" onKeyDown={e => { if (e.key === "Enter" && stampAddLabel.trim()) { setCustomStamps(prev => [...prev, { label: stampAddLabel.trim(), color: stampAddColor }]); setStampAddLabel(""); } }} style={{ width: 80, padding: "4px 8px", borderRadius: 5, border: "1px solid #E0E0DC", fontSize: 12, outline: "none" }} />
                      <input type="color" value={stampAddColor} onChange={e => setStampAddColor(e.target.value)} style={{ width: 26, height: 26, border: "none", padding: 0, cursor: "pointer", borderRadius: 4 }} />
                      <button onClick={() => { if (stampAddLabel.trim()) { setCustomStamps(prev => [...prev, { label: stampAddLabel.trim(), color: stampAddColor }]); setStampAddLabel(""); } }} style={{ padding: "4px 12px", borderRadius: 5, border: "none", background: GREEN, color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>添加</button>
                      <button onClick={() => { setCustomStamps([...QUICK_STAMPS]); }} style={{ padding: "4px 10px", borderRadius: 5, border: "1px solid #E0E0DC", background: "#fff", fontSize: 11, cursor: "pointer", color: "#9CA3AF" }}>恢复默认</button>
                      <div style={{ flex: 1 }} />
                      <button onClick={() => setStampEditing(false)} style={{ padding: "4px 12px", borderRadius: 5, border: "1px solid " + PRIMARY, background: PRIMARY_LIGHT, color: PRIMARY, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>完成</button>
                    </div>}
                  </div>
                </div>
