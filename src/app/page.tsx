...
export default function Home() {
  ...
  function copyOneCorrection(c: any) { clipCopy(c.suggested || c.text, "已复制"); }
          ...
          {activeStudent?.status === "done" && <div style={{ display: "flex", gap: 16 }}>
            ...
            <div style={{ flex: "0 0 42%", overflow: "auto", maxHeight: "calc(100vh - 180px)" }}>
              {activeStudent.essayDetail ? <>
                ...
                <div style={{ marginBottom: 16 }}><h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 10, color: RED }}>✏️ 逐段批注</h3>{(activeStudent.essayDetail.corrections || []).map((c: any, i: number) => (<div key={i} style={{ background: "#fff", borderRadius: 8, padding: 12, marginBottom: 8, border: "1px solid #eee", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}><div style={{ flex: 1 }}><span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4, background: c.type === "praise" ? GREEN : RED, color: "#fff" }}>{c.paragraph}{c.type === "praise" ? " 👍" : ""}</span><p style={{ fontSize: 13, margin: "4px 0 0", color: "#444" }}>{c.text}</p>{c.suggested && c.type !== "praise" && <div style={{ marginTop: 4, padding: "4px 10px", borderRadius: 5, background: "#edf9f1", borderLeft: "3px solid " + GREEN, fontSize: 13, color: GREEN }}>→ {c.suggested}</div>}</div>{c.suggested && <button onClick={() => copyOneCorrection(c)} style={cpBtnS} title="复制建议">📋</button>}</div>))}</div>
